...
# Main function to render the Titan interface
#
# This function sets up the main interface for the Titan Image Generator G1.
...
#
# Parameters:
# - titan_model: An instance of the TitanModel class for image generation

def render_titan(titan_model):
    st.title("Amazon Titan Image Generator G1")
    
    sessions = st.session_state.titan_sessions
    
    session = handle_model_session("titan", sessions, titan_model)
    if not session:
        return

    # Render appropriate step based on session state
    if session['step'] == 'base':
        display_base_step(session, titan_model)
    elif session['step'] == 'variation':
        display_variation_step(session, titan_model)
    elif session['step'] == 'editing':
        display_editing_step(session, titan_model)

# Function to handle the base image generation step
#
# This function creates the interface for generating base images using the Titan model.
...
def display_base_step(session, titan_model):
    ...
    prompt = st.text_area("Text Prompt", key=key_prefix+"base_prompt", max_chars=512, 
                           help="Describe the image you want to generate. Be specific and detailed.")
    negative_prompt = st.text_input("Negative Text Prompt (optional)", key=key_prefix+"negative_prompt", max_chars=512,
    ...
    with col1:
        ...
        sizes = [
            "1024x1024 (1:1)", "768x768 (1:1)", "512x512 (1:1)",
            "1152x896 (Landscape)", "1216x832 (Landscape)", "1344x768 (Landscape)", "1536x640 (Landscape)",
            "1280x768 (Landscape)", "1152x640 (Landscape)", "1173x640 (Landscape)",
            "896x1152 (Portrait)", "832x1216 (Portrait)", "768x1344 (Portrait)", "640x1536 (Portrait)",
            "768x1280 (Portrait)", "640x1152 (Portrait)", "640x1173 (Portrait)"
        ]
    ...
    with col2:
        # Number of images
        num_images = st.slider("Number of Images", 1, 5, 1, key=key_prefix+"number_of_images",
                               help="Select how many images to generate. More images = longer processing time. Default: 1")
    with col3:
        #Seed selection
        seed = st.number_input("Seed", value=0, min_value=0, max_value=2147483646, key=key_prefix+"seed",
    ...
    with st.expander("Advanced Settings"):
        cfg_scale = st.slider("CFG Scale", 1.1, 10.0, 8.0, key=key_prefix+"cfg_scale",
                              help="Determines how much the final image portrays the prompt. Lower values = higher randomness. Default: 8.0")
    ...
    if st.button("Generate Base Image", disabled=not prompt, help="Missing text prompt" if not prompt else ""):
        with st.spinner("Generating image..."):
            images = titan_model.invoke_titan_text_to_image(
                prompt=prompt,
            ...
            if images:
                for image in images:
                    image_key = save_image_to_s3(image, f"titan_sessions/{st.session_state.current_session}/base_images")
                    session['base_images'].append(image_key)
                save_to_s3(st.session_state.titan_sessions, "titan_sessions")
...
def display_variation_step(session, titan_model):
    ...
    with col2:
        prompt = st.text_input("Text Prompt", key=key_prefix+"prompt", max_chars=512,
        ...
        with col1:
            num_images = st.slider("Number of Images", 1, 5, 1, key=key_prefix+"number_of_images",
                                   help="Select how many variations to generate. More images = longer processing time. Default: 1")
        with col2:
            seed = st.number_input("Seed", value=0, min_value=0, max_value=2147483646, key=key_prefix+"seed",
                                   help="Set a seed for reproducible results. Use 0 for random seed. Default: 0")
            if seed == 0:
                seed = random.randint(0, 2147483646)
        similarity_strength = st.slider("Similarity Strength", 0.2, 1.0, 0.7, key=key_prefix+"similarity_strength",
                                        help="Controls how similar the variation is to the original. Higher values = more similar. Default: 0.7")
    ...
    col1, col2 = st.columns(2)
    with col1:
        if st.button("Generate Variations", key="generate_variations", disabled=not prompt, help="Missing text prompt" if not prompt else ""):
            with st.spinner("Generating variations..."):
                ...
                images = titan_model.invoke_titan_image_variation(
                    prompt=prompt,
                    negative_prompt=negative_prompt,
                ...
                if images:
                    for image in images:
                        image_key = save_image_to_s3(image, f"titan_sessions/{st.session_state.current_session}/variation_images")
...
#
# This function creates an interface for editing images using inpainting and outpainting techniques.
...
# - session: The current user session containing state information
# - titan_model: An instance of the TitanModel class for image generation
def display_editing_step(session, titan_model):
    ...
    with col2:
        ...
        with col1:
            num_images = st.slider("Number of Images", 1, 5, 1, key=key_prefix+"number_of_images",
                                   help="Select how many edited versions to generate. More images = longer processing time. Default: 1")
        with col2:
            seed = st.number_input("Seed", value=0, min_value=0, max_value=2147483646, key=key_prefix+"seed",
                                   help="Set a seed for reproducible results. Use 0 for random seed. Default: 0")
            if seed == 0:
        ...
        col1, col2 = st.columns(2)
        with col1:
            editing_mode = st.selectbox("Editing Mode", ["Inpainting", "Outpainting Default", "Outpainting Precise"], key=key_prefix+"editing_mode",
                                        help="Inpainting reconstructs the region within the mask, while Outpainting generates new pixels that seamlessly replace the background or extend the mask region. Outpainting Default allows modification of the image inside the mask in order to keep it consistent with the reconstructed brackground (recommended for non-pixel-level precision). Outpainting Precise prevents modification of image inside the mask.")
        with col2:
    ...
    canvas_result = st_canvas(
        ...
        key=f"canvas_titan_{st.session_state.current_session}",
    )
    ...
    if st.button("Apply Editing", disabled=not prompt, help="Missing text prompt" if not prompt else ""):
        if canvas_result.image_data is not None:
            with st.spinner("Applying edits..."):
                ...
                if editing_mode.startswith("Inpainting"):
                    mask = ~mask
                mask = mask.astype(np.uint8) * 255
                mask = cv2.resize(mask, (original_width, original_height), interpolation=cv2.INTER_NEAREST)
                mask_image = Image.fromarray(mask)

                if editing_mode.startswith("Outpainting"):
                    images = titan_model.invoke_titan_outpainting(
                        prompt=prompt,
                        negative_prompt=negative_prompt,
                        ...
                        seed=seed,
                        cfg_scale=cfg_scale,
                        outpainting_mode="DEFAULT" if editing_mode == "Outpainting Default" else "PRECISE"
                    )
                else:  # Inpainting
                    images = titan_model.invoke_titan_inpainting(
                        prompt=prompt,
                        negative_prompt=negative_prompt,
                        init_image=background_image,
                        mask_image=mask_image,
                ...
                if images:
                    for image in images:
                        image_key = save_image_to_s3(image, f"titan_sessions/{st.session_state.current_session}/editing_images")
